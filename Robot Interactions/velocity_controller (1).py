...
class VelocityController(b_pykdl.baxter_kinematics):
    ...
    def ee_IK_solver(self, joint_values, P_des, quat_des, dt, nullspace_en, vel_elbow):
        ...
        # This transformation is done in the 'ee_ IK_solver' function. 
        # First, information on the current pose is calculated using the forward position kinematcs position method
        # ('self.forward_position_kinematics') from the 'VelocityController'class from earlier, and passing the current joint values
        # ('joint_values') as the input. A vector, 'Pose _ vect' is returned, and its first three elements which represent the position
        # (x, y, z coordinates) are extracted and stored into 'p'. Likewise, the remaining elements of the vector which represent the 
        # orientation as a quaternion (x, y, z, x) are stored in 'quat'
        ...
        # the inverse of the initial orientation quaternion is calculated as 'quat inv, 
        # and its product with thefinal orientation quaternion is computed into 'quat err'       
        ...
        # 'dw'using f (delta angle), 'r' and sample time At ('dt')
        # Subsequently the linear velocity and angular velocity arecombined into a twist vector using 'np . hstack( (dp , dw) )
        ...
        # Compute the Jacobian matrix of the end-effector
        # The Jacobian matrix ('J_ee') represents the relationship between the joint velocities and the end-effector's velocity (twist).
        # It is computed by passing the current joint values ('joint_values') into the self.jacobian() method.
        J_ee = self.jacobian(joint_values)    # your code here, replace [] with the correct variable
        # Convert the Jacobian to a NumPy array
        # The output of self.jacobian(joint_values) may not be in a NumPy array format.
        # To facilitate numerical operations, it is explicitly converted to a NumPy array.
        J_ee = np.asarray(J_ee) # convert to np.array
        # Compute the pseudoinverse of the Jacobian
        # The Jacobian matrix is generally not square, it is often a 6*n matrix, where n is the number of joints
        # To solve for joint velocities in redundant or underdetermined systems, we compute the Moore-Penrose pseudoinverse.
        # The function DPinv(J_ee, eps=1e-10) computes the pseudoinverse of J_ee, 
        # with a small epsilon (eps) to handle numerical stability.
        J_pinv = DPinv(J_ee, eps=1e-10) # your code here, replace [] to compute the pseudoinverse of J_ee
        # Compute the joint velocities using the pseudoinverse method
        # The joint velocities (qd) are obtained by multiplying the pseudoinverse Jacobian (J_pinv) with the desired end-effector velocity (twist).
        ...
        if nullspace_en:
            ...
            # projection in Null space (equation from task F)
            
            ##########################
            #####Task G part ii (delete part i before running)
            ...
            # final task motion is sum of primary motion and seconday motion
...
def main(task):
    ...
    ######################################################
    ## Task C:
    # Define the desired poses for the robot in Cartesian coordinates and rotation.
    # fill in the desired poses
    xyz_des_pick = [0.75, 0, 0.93]       # your code here!# Desired position to pick up the object. The position is set in x, y, z coordinates.
    xyz_des_circle = [0.75, 0.1, 1.23]     # your code here! # Desired position for the circular path. x, y, z coordinates again.
    rpy_des = [0, np.pi/2, 0]  # this is used only for groups. For individuals it has no effect # Desired rotation (roll, pitch, yaw). The rotation is set to pi/2 on the pitch axis.

# This block checks which task is to be performed and applies the corresponding function to calculate the next pose.
    if task == "go2pose":
